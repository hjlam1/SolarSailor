...
public class racerBehaviorB : MonoBehaviour {
	...
	public float boost;
	private float zHeightOffset = 5f;
	...
	void FixedUpdate () {
		...
		float theSpeed = racerVariance * (rubberBand + (Mathf.Cos (Time.timeSinceLevelLoad * 2.5f) * boost));
		...
		movement.z = playerCam.speed / 35 * theSpeed * rubberBand;
